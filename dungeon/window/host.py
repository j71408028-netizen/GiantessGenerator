...
``discard_pending_quit()`` 丢掉「副本视口被原生关闭键关掉」留下的退出类残留消息
                          （Tk 侧是线程队列里的 ``WM_QUIT``，见 §5-C12）
...
class HostPort:
    ...
    def discard_pending_quit(self) -> int:
        """丢掉「副本视口被原生关闭键关掉」留下的退出类残留消息，返回丢弃条数。

        只有视口被**原生关闭键（X）**关掉时才会产生这种消息：GLFW 此时会销毁自己
        的原生窗口，Windows 的 ``DefWindowProc(WM_DESTROY)`` 随之往本线程队列里投一
        条 ``WM_QUIT``。它不会被 Tk 取走，却会让 Windows 停止合成 ``WM_TIMER``——
        Tk 的 ``after`` 计时器与 ``tkwait``（模态对话框）从此拿不到事件，
        表现为「收尾提示框弹出来，整个主窗口卡死」（窗口文档 §5-C12）。

        这条消息只针对那个已经销毁的 GLFW 窗口，与宿主主循环无关，因此丢掉是对的。
        缺省实现（无宿主）什么都不做。
        """
        return 0
